...
class LoadData(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        for i, row in df.iterrows():
            token_list = [Token(x) for x in self.tokenizer(row["text"])]
            label_list = row[["label"]].values

            yield self.text_to_instance(token_list, label_list)
...
def main():
    ...
    # loss.backward()
    #
    # for x in list(model.encoder.parameters()):
    #     x.grad
    #
