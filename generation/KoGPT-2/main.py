...
parser.add_argument('--epoch', type=int, default=10, help="epoch 를 통해서 학습 범위를 조절합니다.")
...
parser.add_argument('--data_file_path', type=str, default='./temp.csv', help="학습할 데이터를 불러오는 경로입니다.")
parser.add_argument('--batch_size', type=int, default=1, help="batch_size 를 지정합니다.")
...
def main(epoch, save_path, load_path, samples, data_file_path, batch_size):
	...
	model_path = download(model_info['url'], model_info['fname'], model_info['chksum'], cachedir=cachedir)
	...
	kogpt2model.train()
	vocab_b_obj = gluonnlp.vocab.BERTVocab.from_sentencepiece(vocab_path, mask_token=None,
